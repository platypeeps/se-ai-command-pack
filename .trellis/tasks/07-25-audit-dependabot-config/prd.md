...
- Enablement: this repo is not a fork, so committing dependabot.yml to the default branch enables version updates automatically — there is no separate repo toggle (verified against GitHub docs + live isFork:false). The only possible blocker is org-level Dependabot disablement, observable only post-merge. AC1 does not depend on that post-merge observation: it is met pre-archive by the strictly-checked committed config plus CONTRIBUTING documentation. The post-merge pickup / observed-blocker check is the post-archive handoff below, not acceptance evidence (see design D6).
