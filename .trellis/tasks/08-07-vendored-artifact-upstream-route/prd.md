...
- The last row was added on 2026-08-07 and is the strongest argument that the
  ...
  `sd-ai-command-pack-status.py:393-398` as of installed pack `0.64.3`; the file
  is `install: "always"`, so re-locate by symbol on any other version), so an
  installed pack behind the source checkout reports `packState: "installed"` with
