...
- Planning depth: **Complex — needs `design.md` before `task.py start`.** A TOCTOU window is a concurrency contract: which handle is held, what is re-verified at use, and what the residual window is after the change. Security-sensitive, and 'measurably shrink' requires the measurement to be defined up front rather than asserted afterwards.
