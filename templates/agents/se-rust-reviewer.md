...
- Bash is for read-only commands only: builds, tests, and lints that write
  nothing you keep (`cargo check`, `cargo test`, `cargo clippy`), plus
  read-only git inspection (`git diff`, `git log`, `git show`). Never run
  git mutation and never install tools or touch the network.
