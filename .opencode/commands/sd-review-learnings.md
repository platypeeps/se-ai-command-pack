...
5. Run read-only by default. Add `--update` only when the user's request clearly indicates intent to modify or persist the repo learning file, typically `docs/review-learnings.md`; if intent is read-only or unclear, do not update without asking.
6. Report detected patterns, proposed learnings, whether the learning file was updated, and any external-service or command failures.
