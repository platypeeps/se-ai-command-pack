---
description: Take the current branch from committed work to a merged pull request by sequencing the standard SD create-pr, routed review, lifecycle, internal check watch, and housekeeping stages.
---
...
Run the Software Delivery (SD) ship workflow. Take the current feature branch through the standard publish-to-merge chain — create or reuse the pull request, run the review loop, watch the pull request with the skill's internal read-only coordinator until it settles, then merge through the housekeeping gate — stopping at the `until=pr|review|merge` stop-point (default `merge`) or immediately with a failed or blocked stage's report.
...
3. Use the skill as the primary instructions. It defines the fixed five-stage chain: the sd-create-pr flow, the routed sd-review scope=pr loop, the composite-owned Stage 2b lifecycle step, the skill's internal read-only watch coordinator, and the sd-housekeeping merge gate, with each stage's own preconditions, gates, and reports staying authoritative, stop-points between stages, and a failed or blocked stage stopping the chain with that stage's report. Pass the `until=pr|review|merge` stop-point and stage arguments such as `timeout-minutes=N` through to the skill.
