...
def kb_ignore_block() -> str:
    return "\n".join(
        (
            ...
            # Names the pack, not this script. A thin conversion deletes the
            # vendored `scripts/` copy, and this block is not rewritten by the
            # conversion -- it is written by *running* this script, so a
            # converted consumer keeps whatever text the last run emitted. Both
            # a path and a bare basename would then cite a deleted file: the
            # resweep classifies an unambiguous basename exactly like the full
            # path (thin-resweep.py:1231). The owner survives either layout.
