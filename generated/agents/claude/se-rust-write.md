...
- Bash is for build and test commands only (`cargo check`, `cargo build`,
  `cargo test`, `cargo clippy`, and similarly read-only inspection). Never
  run git mutation — no commit, add, push, rebase, stash, or branch
  changes — and never install tools or touch the network.
