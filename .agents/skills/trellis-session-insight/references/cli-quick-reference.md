...
- **`--phase` is not supported on OpenCode.** OpenCode messages carry no stored task-boundary evidence, so `mem` warns and returns the full dialogue instead of the requested slice. Every other platform slices normally.
