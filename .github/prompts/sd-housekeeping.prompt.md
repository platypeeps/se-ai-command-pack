...
3. Verify that `~/.agents/bin/sd-ai-command-pack-housekeeping.sh` is readable at that path. Do not probe `PATH`: a `PATH` entry can name a different install than the one the running skill text came from. If the resolved skill is missing, ambiguous, fails validation, or requires unavailable tools, stop and report the skill blocker. If the script is missing, unreadable, empty, fails `bash -n`, or cannot execute, stop and report the script blocker.
4. After following the skill, run `bash ~/.agents/bin/sd-ai-command-pack-housekeeping.sh --finish-work-receipt "$FINISH_WORK_RECEIPT"` only with the unchanged private JSON receipt produced by finish-work for the exact pushed and green head. The eligibility evaluator independently recomputes that receipt before merge. For cleanup-only or already-merged state, run `bash ~/.agents/bin/sd-ai-command-pack-housekeeping.sh` without the option. Capture the exit status and output. The user's housekeeping request authorizes the script's documented verified merge/cleanup flow; ask again only for operations outside that flow. Before any merge, branch deletion, or other irreversible remote operation, ensure the script has verified explicit criteria: the relevant PR is open and green before merge, or confirmed merged with matching branch heads before deletion. Treat the state as ambiguous if PR metadata is missing, more than one PR matches the branch, required checks are absent or non-green, branch heads differ, the working tree is dirty, or default branch detection fails. If state is ambiguous or any command exits nonzero, stop and report the exact command, exit status, complete stdout/stderr, and `git status -sb`.
