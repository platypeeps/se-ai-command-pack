...
3. Verify that `~/.agents/bin/sd-ai-command-pack-housekeeping.sh` is resolvable and readable, either as a bare command on `PATH` or as a file at that path relative to the repository root. If the resolved skill is missing, ambiguous, fails validation, or requires unavailable tools, stop and report the skill blocker. If the script is missing, unreadable, empty, fails `bash -n`, or cannot execute, stop and report the script blocker.
