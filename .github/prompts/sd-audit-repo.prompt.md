...
2. Verify that the charter directory is resolvable and non-empty, either inside the installed skill payload or at `.agents/skills/sd-audit-repo/charters/` under the same root as that payload — the repository root for a vendored install, `$HOME` for a thin one. A thin install carries no repository-relative copy, so a missing repo-root path is not by itself a blocker. If the skill or charter directory is missing, unreadable, empty, resolves to more than one candidate, fails validation, defines contradictory steps that violate this command's safety rules, or requires unavailable tools, stop and report the exact blocker.
3. Use the skill as the primary instructions. It defines the fixed audit pipeline: deterministic applicability preflight, per-dimension reviewer dispatch, adversarial verification, synthesis, Trellis reconciliation, and report plus ledger update. Pass the user's invocation arguments through unchanged; the skill accepts additive bare charter names plus the explicit controls `dimensions=...`, `depth=standard|exhaustive`, and `follow-up`.
...
5. If the applicability classifier is unavailable or uncertain, fall back to exhaustive charter coverage and record the reason. If any charter read, reviewer dispatch, verification pass, ledger read or write, git command, or final validation fails, mark the affected charter failed where applicable, stop, and report the command, exit status, and complete stdout/stderr output.
