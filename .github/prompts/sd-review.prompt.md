...
2. Verify that `~/.agents/bin/sd-ai-command-pack-review.py` and `~/.agents/bin/sd-ai-command-pack-toolchain.sh` are resolvable, either as bare commands on `PATH` or as regular readable files at those paths relative to the repository root. If the skill or either script is missing, malformed, ambiguous, unsafe, or unavailable, stop and report the exact blocker.
