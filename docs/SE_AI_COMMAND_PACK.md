...
| `provenance.json` | `{pack, version, sourceRoot, files: {target: "sha256:..."}}`. Only vouchable results (created/updated/unchanged/overwritten) are recorded; receipts themselves are never vouched. A normal refresh may report differing regular-file bytes as `updated` only when they still match the prior provenance hash. Targets governed by preservation semantics remain `preserved`; those policies take precedence over the vouched update path. `sourceRoot` is the checkout the install ran from — `install.py update` uses it to run updates. Because that path drives `git` and re-executes `install.py`, update trusts it only when it is a git repository (current-user-owned on POSIX) and is the running checkout; a relocated source is refused before any `git`/exec unless confirmed with `--confirm-source` (or interactively). A symlinked `.git` or `install.py`, and a `.git` file whose `gitdir` target is missing, not a directory, or not current-user-owned, are likewise refused; on POSIX the checks and the later `git`/exec run against one directory descriptor opened before the checks, so swapping the path after validation cannot redirect them. |
